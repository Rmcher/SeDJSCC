...
def get_dataset_snr(snr_db):
    ...
    # labels have been packeted into ds
    train_ds = tf.data.Dataset.from_tensor_slices(((x_train, snrdb_train), y_train))
    test_ds = tf.data.Dataset.from_tensor_slices(((x_test, snrdb_test), y_test))
...
def get_dataset_snr_and_h(snr_db):
    ...
    train_ds = tf.data.Dataset.from_tensor_slices(((x_train, snrdb_train, h_real_train, h_imag_train), y_train))
    test_ds = tf.data.Dataset.from_tensor_slices(((x_test, snrdb_test, h_real_test, h_imag_test), y_test))
    return (train_ds, train_nums), (test_ds, test_nums)
